...
void runCuda() {
  ...
  if (iteration == ui_iterations) {
    if (showPerf) {
      std::cout << "Render time for " << ui_iterations << " is " << renderTime
                << std::endl;
      showPerf = false;
    }
    if (ui_denoise && ui_denoiseDirty) {
      ...
      auto timeSpan = std::chrono::duration_cast<std::chrono::duration<double>>(
          endTime - startTime);
      std::cout << "Denoise time for " << ui_filterSize << " filter levels  is "
                << timeSpan.count() << std::endl;
