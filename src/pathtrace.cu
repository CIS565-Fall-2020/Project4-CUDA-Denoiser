...
static glm::vec3 * dev_denoisedBuffer = NULL;		// An image buffer for denoising
...
void pathtraceInit(Scene *scene) {
	...
	cudaMalloc(&dev_denoisedBuffer, pixelcount * sizeof(glm::vec3));
	cudaMemset(dev_denoisedBuffer, 0, pixelcount * sizeof(glm::vec3));
	...
	,glm::vec3 *imageAverage)
{
	...
	if (x < cam.resolution.x && y < cam.resolution.y) {
		...
		glm::vec3 curr_pixel = image[idx];
		curr_pixel /= iter;
		curr_pixel = glm::clamp(curr_pixel, glm::vec3(0.f), glm::vec3(1.f));

		imageAverage[idx] = curr_pixel;
	}
}
...
__global__ void ATrousFilter(
	...
	if (x < cam.resolution.x && y < cam.resolution.y) {
		float h_lib[5] = { 1.f / 16.f, 1.f / 4.f, 1.f / 8.f, 1.f / 4.f, 1.f / 16.f };
		int idx = x+ y * cam.resolution.x;
		...
		for (int i = 0; i < 5; ++i)
		{
			for (int j = 0; j < 5; ++j)
			{
				glm::ivec2 uv = glm::ivec2(x + (i - 2) * stepWidth, y + (j - 2) * stepWidth);
				uv = glm::clamp(uv, glm::ivec2(0, 0), glm::ivec2(cam.resolution.x-1,cam.resolution.y-1));
				int idx_uv = uv.x + uv.y * cam.resolution.x;

				//Color weight
				glm::vec3 t = image[idx] - image[idx_uv];
				float dist2 = glm::dot(t, t);
				float weight_c = glm::min(glm::exp(-dist2 / color_phi), 1.f);

				//Normal weight
				t = gBuffer[idx_uv].normal - gBuffer[idx].normal;
				dist2 = glm::max(glm::dot(t, t) / (stepWidth, stepWidth), 0.f);
				float weight_n = glm::min(glm::exp(-dist2 / normal_phi), 1.f);

				//Position weight
				t = gBuffer[idx_uv].position - gBuffer[idx].position;
				dist2 = glm::dot(t, t);
				float weight_p = glm::min(glm::exp(-dist2 / pos_phi), 1.f);

				float weight = weight_c * weight_n * weight_p;
				sum += image[idx_uv] * weight * h_lib[i] * h_lib[j];
				sum_weight += weight * h_lib[i] * h_lib[j];
			}
		}
		imageDenoised[idx] = sum / sum_weight;
	}
}


void denoise(int iter) {
	const Camera &cam = hst_scene->state.camera;
	const int pixelcount = cam.resolution.x * cam.resolution.y;

	// 2D block for denoising 
	...
	//Average the image 
	average << < blocksPerGrid2d, blockSize2d >> > (
		iter
		, cam
		, dev_image
		, dev_denoised);
	...
	int stepWidth = 1;
	for (int i = 1; i <= ui_filterLayers; ++i)
	{
		...
		std::swap(dev_denoised, dev_denoisedBuffer);
	}
...
void showDenoisedImage(uchar4 * pbo)
{
	...
	// Send results to OpenGL buffer for rendering
	sendImageToPBO << <blocksPerGrid2d, blockSize2d >> > (pbo, cam.resolution, 1, dev_denoised);
}
